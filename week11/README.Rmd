...
---
**See my lab output** [**here**](https://ghcdn.rawgit.org/aomidsalar/PM566_Labs/main/week11/README.html)
...
library(widgetframe)
library(zoo)
##ghcdn.rawgit.org and add html url
```
...
*The data looks to be in the correct formatting. It has been reordered alphabetically by state and in chronological order, so the earlier observations from Alabama appear from the `head` command, and the more recent observations from Wyoming appear from the `tail` command.*
...
*The date range is from January 1, 2020 to the present day (`rmax(cv_states$date)[[1]]`). The range of cases is from 24224 to 5007632. The range of deaths is 255 to 74189.*
...
*After getting the rolling averages, the graph looks a lot more smooth and is easier to read. Florida and Texas have the highest new deaths, and Oklahoma had a recent spike in October 2021.*
...
*Washington DC was removed from the scatterplot because it was a major outlier with high population density. Once that was removed, it was a lot easier to see the differences between the state-level data. New Jersey, Rhode Island, Massachusetts, and Connecticut stand out to me as states with both a relatively high population density and deaths per 100k.*
...
# filter out "District of Columbia", major outlier
...
*There appears to be a slight positive correlation between `pop_density` and `newdeathsper100k`, but that is likely due to the outlier states with the high population density pulling the best fit curve upwards. Between a `pop_density` of 1 and 250, there does not appear to be a clear positive or negative correlation.*
...
*States such as Louisiana, Hawaii, South Carolina, and Florida had an increase in September. Hawaii had interesting variations in naive_CFR where it decreased around mid-July and increased in September, plateauing out in late October and November.*
...
*The approximate peak of cases is August 16, 2021 (with a second peak on September 13, 2021) and the approximate peak of deaths is September 20, 2021. There appears to be about a month difference between these two values.*
...
Create a heatmap to visualize `new_cases` for each state on each date greater than June 15th, 2021
...
*States that have large populations or with large outbreaks stand out (California, Florida, Texas)*

- Repeat with `newper100k` variable. Now which states stand out? 

*Now, there are more states standing out; for example, Alaska had a high case rate, as well as some southern states (Mississippi, Florida, Louisiana, South Carolina, Tennessee).*
...
*The southern states mentioned previously as well as Alaska stand out to me here.*
...
*Overall, the maximum national CFR appears to have decreased from October 15th to the most recent date. This is most evident in Alaska which had 123.7 cases per 100k on October 15th, and that has decreased more than half (58.7). On the other hand, there are some states experiencing increases between the two dates (New Mexico, Arizona, Vermont, and New Hampshire). *
...
fig <- fig %>% colorbar(title = paste0("Cases per 100k: "), limits = c(0,shadeLimit))
fig <- fig %>% layout(
...
fig_Today <- fig %>% hide_colorbar()

### Plot together 
s1 <- subplot(fig_pick.date, fig_Today, nrows=2)%>%  
  layout(annotations = list(
    list(text = Sys.Date(), showarrow = F, xref='paper', yref='paper')) 
  ) 
s1
